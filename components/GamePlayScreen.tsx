...
export default function GamePlayScreen() {
  ...
  // sample card (replace with backend later)
  const card = [
    [5, 18, 31, 52, 62],
    [10, 21, 43, 56, 66],
    [9, 28, "FREE", 59, 69],
  ...
  return (
    <div className="p-3 max-w-md mx-auto text-white">
      ...
      <div className="flex justify-between items-center mb-3">
        ...
        <p className="text-sm text-gray-400">Stake: 10 ETB</p>
      </div>
      ...
      {/* BINGO BUTTON */}
      <button className="w-full bg-orange-500 py-3 rounded-xl font-bold text-lg mb-3 active:scale-95">
        BINGO!
      </button>

      {/* ACTION BUTTONS */}
      <div className="flex gap-2">
        <button className="flex-1 bg-blue-500 py-2 rounded">Refresh</button>
        <button className="flex-1 bg-red-500 py-2 rounded">Leave</button>
      </div>
    </div>
  );
}
